...
\subsubsection{\circled{1} Home page}

\begin{tblr}{colspec={lX}, row{1}={lightestgray}}

\textit{Algorithm name:} & {\scshape Home\_Page} \\

\textit{Description:} & {This function demonstrates how the user will interact
with the home page of our system.} \\

\textit{Justification:} & {The home page is meant to grab the user's attention
and entice them into trying out our system.}\\

\end{tblr}

\begin{center}
\begin{tikzpicture}[
  base/.style = {draw, text centered, minimum width=8em, minimum height=3em}
]
  \node (start) [base, terminal] {Start};
  \node (enter) [base, rectangle, below=of start] {User enters website URL};
  \node (anchor) [below=of enter] {};
  \node (help)  [base, predproc, right=of anchor] {Help page};
  \node (reg)   [base, predproc, left=of anchor] {Registration page};
  \node (sign)  [base, predproc, right=of help] {Login up page};
  \node (doc)   [base, predproc, left=of reg] {Documentation page};
  \node (bot)   [below=of anchor] {};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (enter);
  \draw[draw, -{Latex[length=2.5mm]}] (enter) -| (help);
  \draw[draw, -{Latex[length=2.5mm]}] (enter) -| (reg);
  \draw[draw, -{Latex[length=2.5mm]}] (enter) -| (sign);
  \draw[draw, -{Latex[length=2.5mm]}] (enter) -| (doc);
  \draw[draw, -{Latex[length=2.5mm]}] (doc.south)   |- (bot);
  \draw[draw, -{Latex[length=2.5mm]}] (reg.south)   |- (bot);
  \draw[draw, -{Latex[length=2.5mm]}] (help.south)  |- (bot);
  \draw[draw, -{Latex[length=2.5mm]}] (sign.south)  |- (bot);
  \draw[draw, -{Latex[length=2.5mm]}] (bot.north)  -- (enter.south);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{1.1} Help page}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (choose)[base, decision, below=of start] {Select an option};
  \node (guide) [base, process, below right=of choose] {System guide};
  \node (faq)   [base, process, below left=of choose] {FAQs};
  \node (end)   [base, terminal, below=of choose] {End};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (choose);
  \draw[draw, -{Latex[length=2.5mm]}] (choose) -| (guide);
  \draw[draw, -{Latex[length=2.5mm]}] (choose) -| (faq);
  \draw[draw, -{Latex[length=2.5mm]}] (faq) |- (end);
  \draw[draw, -{Latex[length=2.5mm]}] (guide) |- (end);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{1.2} Login page}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (name)  [base, process, below=of start] {Enters username};
  \node (pass)  [base, process, below=of name] {Enters password};
  \node (login) [base, predproc, below=of pass] {Log\_User\_In};
  \node (end)   [base, predproc, below=of login] {Main\_Page};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (name);
  \draw[draw, -{Latex[length=2.5mm]}] (name) -- (pass);
  \draw[draw, -{Latex[length=2.5mm]}] (pass) -- (login);
  \draw[draw, -{Latex[length=2.5mm]}] (login) -- (end);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{1.3} Documentation}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (read)  [base, process, below=of start]  {Read through documentation};
  \node (end)   [base, terminal, below=of read] {End};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (read);
  \draw[draw, -{Latex[length=2.5mm]}] (read) -- (end);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{2} Validate username and password}
...
\subsubsection{\circled{2} / \circled{4} Account creation}
...
\begin{algorithmic}[1]
  \Function{Add\_Account}{Username, Password}
    \If{\textbf{not} Validate\_Account(Username, Password)} \State{return False}
    \EndIf
...
\subsubsection{\circled{3} Log user in}
...
\subsubsection{\circled{5} Main page}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (create)[base, predproc, below=of start] {Create call};
  \node (join)  [base, predproc, right=of create] {Join call};
  \node (sett)  [base, predproc, left=of create] {Settings};
  \node (end)   [base, terminal, below=of create] {End};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (create);
  \draw[draw, -{Latex[length=2.5mm]}] (start.east) -| (join.north);
  \draw[draw, -{Latex[length=2.5mm]}] (start.west) -| (sett.north);
  \draw[draw, -{Latex[length=2.5mm]}] (create) -- (end);
  \draw[draw, -{Latex[length=2.5mm]}] (sett) |- (end);
  \draw[draw, -{Latex[length=2.5mm]}] (join) |- (end);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{6} Settings page}

\begin{tblr}{colspec={lX}, row{1}={lightestgray}}

\textit{Algorithm name:} & {\scshape Settings\_Page}\\

\textit{Description:} & {This will be the settings page where users can configure their video
conferencing experience to fit their needs}\\

\textit{Justification:} & {Users should be able to customise their video conferencing experience. This
contributes to improving the accessibility of our website, directly accomplishing one of client's main
requests. (Refer to section \ref{sec:interview})}\\
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (audio) [base, predproc, below=of start] {Audio};
  \node (video) [base, predproc, right=of audio] {Video};
  \node (acc)   [base, predproc, left=of audio] {Accessibility};
  \node (end)   [base, predproc, below=of audio] {Main\_Page};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (audio);
  \draw[draw, -{Latex[length=2.5mm]}] (start) -| (video);
  \draw[draw, -{Latex[length=2.5mm]}] (start) -| (acc);
  \draw[draw, -{Latex[length=2.5mm]}] (audio) -- (end);
  \draw[draw, -{Latex[length=2.5mm]}] (acc) |- (end);
  \draw[draw, -{Latex[length=2.5mm]}] (video) |- (end);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{7} Create call page}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (code)  [base, io, below=of start] {Code given to user};
  \node (call)  [base, predproc, below=of code] {Call starts};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (code);
  \draw[draw, -{Latex[length=2.5mm]}] (code) -- (call);
\end{tikzpicture}
\end{center}

\subsubsection{\circled{8} Join call page}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (code)  [base, io, below=of start] {User enters code};
  \node (call)  [base, predproc, below=of code] {Call starts};
...
\subsubsection{\circled{9} Audio settings}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (conf)  [base, process, below=of start] {User changes audio settings};
  \node (choose)[base, decision, below=of conf] {Save changes?};
  \node (save)  [base, predproc, below=of choose] {Save settings to \textit{DB}};

  \node (sr) [right=2cm of start] {};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (conf);
  \draw[draw, -{Latex[length=2.5mm]}] (conf) -- (choose);
  \draw[draw, -{Latex[length=2.5mm]}] (choose) edge["Yes"] (save);

  \draw[draw, -{Latex[length=2.5mm]}]
    (choose.east) -| node[right] {No} (sr) -- (start.east);
...
\subsubsection{\circled{10} Video settings}

\begin{tblr}{colspec={lX}, row{1}={lightestgray}}

\textit{Algorithm name:} & {\scshape Video\_Settings\_Page}\\

\textit{Description:} & {This will be the page from which users will be able to configure their video and resolution
settings}\\

\textit{Justification:} & {Users should be able to configure their video settings so that they can ensure that
their video conferencing experience is the best given their device hardware.}\\
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (conf)  [base, process, below=of start] {User changes video settings};
  \node (choose)[base, decision, below=of conf] {Save changes?};
  \node (save)  [base, predproc, below=of choose] {Save settings to \textit{DB}};
...
\subsubsection{\circled{11} Accessibility settings}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (conf)  [base, process, below=of start] {User changes accessibility settings};
  \node (choose)[base, decision, below=of conf] {Save changes?};
  \node (save)  [base, predproc, below=of choose] {Save settings to \textit{DB}};
...
\subsubsection{\circled{12} Invite others}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (give)  [base, process, below=of start] {User to be invited is given a code};
  \node (nav)   [base, process, below=of give] {User to be invited navigates to join call page};
  \node (join)  [base, predproc, below=of nav] {Connect\_To\_Call};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (give);
  \draw[draw, -{Latex[length=2.5mm]}] (give) -- (nav);
  \draw[draw, -{Latex[length=2.5mm]}] (nav) -- (join);
\end{tikzpicture}
\end{center}
...
\subsubsection{\circled{13} Enter pass code}

\begin{tblr}{colspec={lX}, row{1}={lightestgray}}

\textit{Algorithm name:} & {\scshape Enter\_Passcode}\\

\textit{Description:} & {This algorithm describes the series of steps required to enter in a 6 digit call code and
join a video conference}\\

\textit{Justification:} & {We need to verify whether or not a call actually exists with a certain code, then if so
we will connect a user to this video conference. This allows users to video conference and communicate with each other
directly satisfying one of our client's main requests.}\\
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (enter) [base, io, below=of start] {User enters code};
  \node (check) [base, decision, below=of enter, text width=8em] {Check whether a video call exists with this call code};
  \node (conn)  [base, predproc, below=of check] {Join\_Call};

  \node (sr) [right=3cm of start] {};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (enter);
  \draw[draw, -{Latex[length=2.5mm]}] (enter) -- (check);
  \draw[draw, -{Latex[length=2.5mm]}] (check) edge["Yes"] (conn);
  \draw[draw, -{Latex[length=2.5mm]}] (check.east) -| node[right] {No} (sr) -- (start.east);
\end{tikzpicture}
\end{center}

\textit{Iterative test plan: (Assuming only 1 call exists with the code \texttt{123456}} \\ \vspace{0.2cm}
...
Normal & Enter in the call code \texttt{123456} & The \code{Join\_Call()} function successfully runs & The system
should allow users to join video conferences provided they enter in the correct call code\\

Erroneous & Enter in the call code \texttt{111111} & The \code{Join\_Call()} function doesn't run and an error page is successfully displayed & The system shouldn't allow users to join video conferences if they enter in the wrong code\\

\hline
\end{tblr}

\subsubsection{\circled{14} Save settings to DB}
...
\subsubsection{\circled{15} Join call}
...
\subsubsection{\circled{15} Create call}
...
\subsubsection{\circled{16} Raise error}
...
\begin{tikzpicture}[
  ...
  \node (start) [base, terminal] {Start};
  \node (raise) [base, process, below=of start] {Display call not found page};
  \node (end)   [base, terminal, below=of raise] {End};

  \draw[draw, -{Latex[length=2.5mm]}] (start) -- (raise);
  \draw[draw, -{Latex[length=2.5mm]}] (raise) -- (end);
\end{tikzpicture}
\end{center}
