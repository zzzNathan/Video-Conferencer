...
\begin{itemize}
  \item Video feed is working \\
  \item Audio feed is working \\
  \item Video call runs without crashing \\
\end{itemize}

{\sffamily Evidence:} \\ \vspace{0.2cm}
...
This produced the following, \\ \vspace{0.2cm}

{\sffamily Evidence:} \url{https://youtu.be/p1BOWjBdH0g}

The next thing to think about was what settings we should
include and how to implement them respectively. For the
settings that relate to video conferencing since we are
making use of the GetStream video conferencing SDK we would
have to provide settings that are compatible with the
features that the SDK provides. The full list of options
and configurations for video conferences is availbable at
\url{https://getstream.io/video/docs/api/call_types/settings/}.
