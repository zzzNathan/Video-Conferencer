...
Given that our system will have to make use of a database we 
need to select what database system we want to use and we also
need somewhere to host this database. There are exactly 2 
requirements for this database system as follows. The database 
for this system must be held in the cloud, the reasoning for 
this decision is based on the fact that both me and my client
don't own any servers on which we could host the database on. 
Secondly, the database hosting should be free
\textit{if this is at all possible}, to be in line with my 
client's desires for this system. It turns out that there are
free database hosting systems available provided that users do
not exceed some limit in storage capacity, however the 2 
requirements given above do severely restrict our options. The
following table provides information on some of the free 
database hosting systems I found with independent research 
\footnote{Source:
...
matches $m$ in the sequence. More formally the algorithm
computes
